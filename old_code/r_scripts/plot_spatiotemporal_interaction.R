...
## GLENELG
# fox
data_g_plot_fox <- cbind(data_g_plot, predict.gam(gam_fox_sp, newdata = data_g_plot, se.fit = FALSE, type = "terms", terms = "ti(hour,x,y)"))
data_g_plot_fox$species <- "fox"
# cat
data_g_plot_cat <- cbind(data_g_plot, predict.gam(gam_cat_sp, newdata = data_g_plot, se.fit = FALSE, type = "terms", terms = "ti(hour,x,y)"))
...
## OTWAYS
# fox
data_o_plot_fox <- cbind(data_o_plot, predict.gam(gam_fox_sp, newdata = data_o_plot, se.fit = FALSE, type = "terms", terms = "ti(hour,x,y)"))
data_o_plot_fox$species <- "fox"
# cat
data_o_plot_cat <- cbind(data_o_plot, predict.gam(gam_cat_sp, newdata = data_o_plot, se.fit = FALSE, type = "terms", terms = "ti(hour,x,y)"))
...
fox_hour / plot_facet_select_g_fox / plot_facet_select_o_fox + plot_annotation(title = "a.   Fox")
dev.off()

png("figs/spatial_interaction_cat.png", width = 8, height = 7, res = 600, units = "in")
cat_hour / plot_facet_select_g_cat / plot_facet_select_o_cat + plot_annotation(title = "b.   Feral cat")
dev.off()
