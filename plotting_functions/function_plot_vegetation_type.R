plot_model_2 <- function(gam_model, data){
  
  ## Plot global smooth (using the gratia package)
  plot_global <- draw(smooth_estimates(gam_model, overall_uncertainty = TRUE, "s(hour)"), xlab = "Hour",  title = "", subtitle = "i.   Average") + 
  ...
  plot_difference <- draw(smooth_estimates(gam_model, "s(hour,vegetation_group)"), xlab = "Hour",  title = "", subtitle = "ii.   Vegetation-specific deviations from the average") + 
    geom_vline(xintercept = c(6.16,18.34), colour = "black", linewidth = 0.6, linetype="dotted") 
  ...
  ## Assemble plots
  print((plot_global | plot_difference) / plot_veg)
}