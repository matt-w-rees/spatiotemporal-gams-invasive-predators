plot_model_3 <- function(){
  
  ## CAT PLOT 3D  
  # new dataframe to predict into
  df = expand.grid(hour = 0:23,
                   habitat_type = levels(records$habitat_type),
                  # longitude = mean(records$x),
                  # latitude = mean(records$y),
                   fox_count_adj = seq(min(records$fox_count_adj), max(records$fox_count_adj), length=150),
                   station = "T052",
                   survey_duration = 60)
  # predict model results into dataframe
  df_int <- cbind(df, predict(gam_cat_fox, newdata = df, se.fit = TRUE, type = "link", exclude = c("s(station)", "s(survey_duration)")))
  # plot
  ...
  # return plot
  print(plot_fox / plot_range / plot_wet_lowhigh + plot_annotation(tag_levels = "a"))
