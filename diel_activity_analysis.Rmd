...
data_g_plot <- cbind(data_g_plot, predict.gam(gam_fox_sp, newdata = data_g_plot, se.fit = TRUE, type = "response", exclude = c("s(station)", "s(survey_duration)")))
...
data_g_plot <- data_g_plot %>% 
  ...
  mutate(diel_diff = max(fox_predicted) - min(fox_predicted),          
         diel_diff_pc = diel_diff / min(fox_predicted) * 100)
...
g_fox_plot_hr <- ggplot(aes(x, y, fill = fox_predicted),
  ...
  geom_point(data = records_g, fill = NA, col = "white", size = 0.7, alpha = 1, shape = 3) +
  theme_bw(10) + 
...
g_fox_plot_diff <- ggplot(aes(x, y, fill = diel_diff_pc),
                     data = data_g_plot) +
  geom_tile()+
  scale_fill_viridis("Relative activity increase (%)", option = "viridis", limits = c(76, 371)) +
  geom_point(data = records_g, fill = NA, col = "white", size = 0.7, alpha = 1, shape = 3) +
  theme_bw(10) + 
...
data_o_plot <- cbind(data_o_plot, predict.gam(gam_fox_sp, newdata = data_o_plot, se.fit = TRUE, type = "response", exclude = c("s(station)", "s(survey_duration)")))
...
o_fox_plot_hr <- ggplot(aes(x, y, fill = fox_predicted),
  ...
  geom_tile()+
  scale_fill_viridis("activity", option = "viridis") +
  geom_point(data = records_o, fill = NA, col = "white", size = 0.7, alpha = 1, shape = 3) +
  theme_bw(10) + 
  theme(axis.title = element_blank()) + 
...
# fox plot: spatial difference
o_fox_plot_diff <- ggplot(aes(x, y, fill = diel_diff_pc),
                     data = data_o_plot) +
  geom_tile()+
  scale_fill_viridis("Relative activity increase (%)", option = "viridis", limits = c(76, 371)) +
  geom_point(data = records_o, fill = NA, col = "white", size = 0.7, alpha = 1, shape = 3) +
...
### GLENELG
# predict model results into dataframe
data_g_plot <- cbind(data_g_plot, predict.gam(gam_cat_sp, newdata = data_g_plot, se.fit = TRUE, type = "response", exclude = c("s(station)", "s(survey_duration)")))
...
data_g_plot <- data_g_plot %>% 
  ...
  mutate(diel_diff = max(cat_predicted) - min(cat_predicted),          
         diel_diff_pc = diel_diff / min(cat_predicted) * 100)
...
g_cat_plot_diff <- ggplot(aes(x, y, fill = diel_diff_pc),
                     data = data_g_plot) +
  geom_tile()+
  scale_fill_viridis("Relative activity increase (%)", option = "viridis", limits = c(76, 371)) +
  geom_point(data = records_g, fill = NA, col = "white", size = 0.7, alpha = 1, shape = 3) +
...
### OTWAYS
# predict model results into dataframe
data_o_plot <- cbind(data_o_plot, predict.gam(gam_cat_sp, newdata = data_o_plot, se.fit = TRUE, type = "response", exclude = c("s(station)", "s(survey_duration)")))
...
# cat plot: spatial difference
o_cat_plot_diff <- ggplot(aes(x, y, fill = diel_diff_pc),
                     data = data_o_plot) +
  geom_tile()+
  scale_fill_viridis("Relative activity increase (%)", option = "viridis", limits = c(76, 371)) +
  geom_point(data = records_o, fill = NA, col = "white", size = 0.7, alpha = 1, shape = 3) +
